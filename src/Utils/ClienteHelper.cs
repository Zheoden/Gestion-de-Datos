    ...
    public partial class DBHelper {
        ...
        public static Boolean altaCliente(Cliente cliente) {
            ...
            comm.CommandText = "INSERT INTO EL_REJUNTE.Cliente (clie_nombre, clie_apellido, clie_tipo_documento, clie_documento, clie_cuil, clie_email, clie_telefono, clie_direccion_id, clie_fecha_nacimiento, clie_fecha_creacion, clie_tarjeta_id, clie_habilitado, clie_usuario_id) " +
                                "VALUES ('" + cliente.nombre + "', '" + cliente.apellido + "', '" + cliente.tipo_documento + "', " + cliente.documento + ", '" + cliente.cuil + "', '" + cliente.mail + "', '" + cliente.telefono + "'," + "(SELECT TOP 1 dire_id FROM EL_REJUNTE.Direccion WHERE dire_calle = '" + cliente.dire.calle + "' AND dire_numero = '" + cliente.dire.numero + "' AND dire_piso = '" + cliente.dire.piso + "' AND dire_depto = '" + cliente.dire.depto + "' AND dire_localidad = '" + cliente.dire.localidad + "' AND dire_codigo_postal = '" + cliente.dire.codigo_postal + "') , " + "GETDATE()" + ", " + "GETDATE()" + "," + "(SELECT TOP 1 tarj_id FROM EL_REJUNTE.Tarjeta WHERE tarj_numero = '" + cliente.tarjeta.numero + "' AND tarj_cod_seguridad = '" + cliente.tarjeta.cod_seguridad + "' AND tarj_vencimiento = '" + cliente.tarjeta.vencimiento + "' AND tarj_titular = '" + cliente.tarjeta.titular + "' AND tarj_tipo = '" + cliente.tarjeta.tipo + "')" + ", 1, null" +
                                ")";
        ...
        public static Cliente getClienteData(int ID) {
            ...
            if (reader.HasRows) {
                while (reader.Read()) {
                    ...
                    cliente.fecha_nacimiento = Convert.ToDateTime(reader["clie_fecha_nacimiento"].ToString());
                    cliente.fecha_creacion = Convert.ToDateTime(reader["clie_fecha_creacion"].ToString());
